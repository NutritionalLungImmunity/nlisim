...
def constant(state: State) -> State:
    ...
    for name in ('concentration', 'diffusivity', 'wind_x', 'wind_y', 'source'):
        ...
        var[:] += state.config.getfloat('simulation.constant_mutation', name, fallback=0)
    return state


def point_source(state: State) -> State:
    """Initialize a single point source somewhere inside the domain."""
    px = state.config.getfloat('simulation.point_source_mutation', 'px', fallback=0.5)
    py = state.config.getfloat('simulation.point_source_mutation', 'py', fallback=0.5)
    value = state.config.getfloat('simulation.point_source_mutation', 'value', fallback=1)
