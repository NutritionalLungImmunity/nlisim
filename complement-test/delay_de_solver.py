...
def make_solver(
    ...
    @jit(nopython=True)
    def make_delay_array(ts: np.ndarray, ys: np.ndarray, delay: float,
                         t_current: float) -> np.ndarray:
        """
        ...
        # between ts[idx-1] and ts[idx]. This could return zero, which can either be an `error`
        # value or the correct answer, when ts[idx] is the correct value.
        idx = np.argmax(ts + delay >= t_current)
        # if we hit the value more-or-less exactly (should be most common case?)
        if np.isclose(ts[idx] + delay, t_current):
            return ys[idx]
        # if ...
        if idx == 0:
            if ts[0] + delay >= t_current:
                return ys[0]
            else:
                return ys[-1]
    ...
    if implicit_euler:
        ...
        def implicit_euler_solver(
            ...
            for prev_idx in range(num_datapoints - 2):
                ...
                implicit_euler_step(ts[next_idx], dt, ys, next_idx)
    ...
    def bdf_solver(
        ...
        implicit_euler_step(ts[next_idx], ts[next_idx] - ts[prev_idx], ys, 1)
        ...
        if remaining_datapoints == 1 and ts[next_idx] + dt > t_end:
            implicit_euler_step(t_np1=t_end, dt=t_end - ts[next_idx], ys=ys, idx=next_idx + 1)
            ts[-1] = t_end
            remaining_datapoints -= 1
        ...
        next_idx += 1  # = 2
        ts[next_idx] = ts[prev_idx] + dt
        bdf2_step(t_np1=ts[next_idx], dt=dt, ys=ys, idx=next_idx)
        remaining_datapoints -= 1
        ...
        while remaining_datapoints > 0:
            ...
            next_idx += 1
            ts[next_idx] = ts[prev_idx] + dt
            bdf3_step(ts[next_idx], dt, ys, next_idx)
            remaining_datapoints -= 1

            # if there is only one step left, and it is irregularly spaced,
            # finish with implicit Euler.
            if remaining_datapoints == 1 and ts[next_idx] + dt > t_end:
                implicit_euler_step(t_end, t_end - ts[next_idx], ys, next_idx + 1)
                ts[-1] = t_end
                remaining_datapoints -= 1
